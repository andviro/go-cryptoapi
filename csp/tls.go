...
func newCredentials(certs []Cert) (res *credentials, err error) {
	...
	if stat := C.AcquireCredentialsHandle_wrap((C.PVOID)(&res.schannelCred), &res.hClientCreds, (*C.TimeStamp)(&res.expires)); stat != C.SEC_E_OK {
...
func (c *Conn) Handshake() (err error) {
	...
	if stat := C.QueryContextAttributes_wrap(&c.hContext, C.SECPKG_ATTR_STREAM_SIZES, (C.PVOID)(c.sizes)); stat != C.SEC_E_OK {
